WITH civil AS (
    SELECT
        ...
        SUM(CASE WHEN c2.type = 'Import' THEN c2.value ELSE 0 END) AS import_value,
        SUM(CASE WHEN c2.type = 'Export' THEN c2.value ELSE 0 END) AS export_value,
        COUNT(DISTINCT c2.goods_id) AS unique_goods_traded,
        COUNT(DISTINCT CASE WHEN c2.type = 'Import' THEN c2.goods_id END) AS unique_imports,
        COUNT(DISTINCT CASE WHEN c2.type = 'Export' THEN c2.goods_id END) AS unique_exports
    FROM
        caravans c
    JOIN
        trade_transactions tt ON c.caravan_id = tt.caravan_id
    JOIN
        caravan_goods c2 ON c.caravan_id = c2.caravan_id
    GROUP BY
...
fortress_resource_dependency AS (
    SELECT
        c2.material_type,
        COUNT(DISTINCT c2.goods_id) AS times_imported,
        SUM(c2.quantity) AS total_imported,
        SUM(c2.value) AS total_import_value,
        COUNT(DISTINCT c.caravan_id) AS caravans_importing,
        AVG(c2.price_fluctuation) AS avg_price_fluctuation,
        (COUNT(DISTINCT c2.goods_id) *
         SUM(c2.quantity) *
    ...
    FROM
        caravan_goods c2
    JOIN
        caravans c ON c2.caravan_id = c.caravan_id
    WHERE
        c2.type = 'Import'
    GROUP BY
        c2.material_type
...
workshop_export_effectiveness AS (
    SELECT
        ...
        COUNT(DISTINCT CASE WHEN c2.goods_id IS NOT NULL THEN p.product_id END) AS products_exported,
        SUM(p.value) AS total_production_value,
        SUM(CASE WHEN c2.goods_id IS NOT NULL THEN c2.value ELSE 0 END) AS export_value,
        AVG(CASE WHEN c2.goods_id IS NOT NULL THEN (c2.value / p.value) ELSE NULL END) AS avg_export_markup
    FROM
        products p
            JOIN
        workshops w ON p.workshop_id = w.workshop_id
            LEFT JOIN
        caravan_goods c2 ON p.product_id = c2.original_product_id AND c2.type = 'Export'
    GROUP BY
        p.type, w.type
),
trade_timeline AS (
...
SELECT
    ...
    (SELECT SUM(total_trade_value) FROM civil) AS all_time_trade_value,
    (SELECT SUM(export_value) - SUM(import_value) FROM civil) AS all_time_trade_balance,
    JSON_OBJECT(
        'civilization_trade_data', (
        SELECT JSON_ARRAYAGG(
            JSON_OBJECT(
                'civilization_type', c3.civilization_type,
                'total_caravans', SUM(c3.caravan_count),
                'total_trade_value', SUM(c3.total_trade_value),
                'trade_balance', SUM(c3.export_value) - SUM(c3.import_value),
                'trade_relationship', CASE
                                          WHEN (SUM(c3.export_value) - SUM(c3.import_value)) > 0 THEN 'Favorable'
                                          WHEN (SUM(c3.export_value) - SUM(c3.import_value)) < 0 THEN 'Unfavorable'
                                          ELSE 'Balanced'
                    END,
                'diplomatic_correlation', dtc.trade_diplomacy_correlation,
                'unique_goods_traded', SUM(c3.unique_goods_traded),
                'years_active', COUNT(DISTINCT c3.trade_year),
                'caravan_ids', (
                    SELECT JSON_ARRAYAGG(c.caravan_id)
                    FROM caravans c
                    WHERE c.civilization_type = c3.civilization_type
                )
            )
        )
        FROM civil c3
            LEFT JOIN diplomatic_trade_correlation dtc ON c3.civilization_type = dtc.civilization_type
        GROUP BY c3.civilization_type, dtc.trade_diplomacy_correlation
    )
        ...
        SELECT JSON_ARRAYAGG(
                       JSON_OBJECT(
                               ...
                               'resource_ids', (
                                   ...
                                   FROM resources r
                                            JOIN caravan_goods c2 ON r.name = c2.material_type
    ...
    JSON_OBJECT(
        ...
        SELECT JSON_OBJECT(
                       'import_to_production_ratio', ROUND(
                                (SELECT SUM(import_value) FROM civil) /
                                NULLIF((SELECT SUM(total_production_value) FROM workshop_export_effectiveness), 0) * 100, 2
                                                     ),
                       'export_to_production_ratio', ROUND(
                                       (SELECT SUM(export_value) FROM civil) /
                                       NULLIF((SELECT SUM(total_production_value) FROM workshop_export_effectiveness), 0) * 100, 2
                                                     ),
                       'trade_dependency_score', ROUND(
                                       (SELECT SUM(total_trade_value) FROM civil) /
                                       (SELECT COUNT(DISTINCT trade_year) FROM civil) /
                           ...
                           SELECT JSON_ARRAYAGG(
                                          JSON_OBJECT(
                                                  ...
                                                  'product_ids', (
                                                      ...
                                                      FROM products p
                                                               JOIN caravan_goods c2 ON p.product_id = c2.original_product_id
                                                      WHERE p.type = x.product_type AND c2.type = 'Export'
                                                  )
                                          )
                                  )
                           FROM (
                       ...
                       'most_expensive_imports', (
                           SELECT JSON_ARRAYAGG(
                                          JSON_OBJECT(
                                                  ...
                                                  'goods_ids', (
                                                      SELECT JSON_ARRAYAGG(c2.goods_id)
                                                      FROM caravan_goods c2
                                                      WHERE c2.material_type = i.material_type AND c2.type = 'Import'
                                                  )
                                          )
                                  )
                           FROM (
                                    SELECT
                                        c2.material_type,
                                        SUM(c2.value) AS import_value
                                    FROM caravan_goods c2
                                    WHERE c2.type = 'Import'
                                    GROUP BY c2.material_type
                                    ORDER BY SUM(c2.value) DESC
                                    LIMIT 5
                                ) i
                       )
               )
        ...
        SELECT JSON_ARRAYAGG(
                       JSON_OBJECT(
                               ...
                               'potential_partners', (
                                   ...
                                   FROM caravans c
                                            JOIN caravan_goods c2 ON c.caravan_id = c2.caravan_id
                                   WHERE c2.material_type = r.material_type AND c2.type = 'Import'
                               ),
                               'resource_ids', (
                ...
                SELECT JSON_ARRAYAGG(
                               JSON_OBJECT(
                                       ...
                                       'potential_value', COALESCE(
                                               (SELECT SUM(p.value)
                                                FROM products p
                                                         ...
                                                         LEFT JOIN caravan_goods c2 ON p.product_id = c2.original_product_id AND c2.type = 'Export'
                                                WHERE w2.type = w.type AND c2.goods_id IS NULL),
                                               0
                                                          ),
                                       'recommended_civilizations', (
                                           ...
                                           FROM caravans c
                                                    JOIN caravan_goods c2 ON c.caravan_id = c2.caravan_id
                                           WHERE c2.type = 'Import' AND c.civilization_type IN (
