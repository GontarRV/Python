Задания 18
Задание 18.1
SELECT Dwarves.dwarf_id, Dwarves.name AS dwarf_name, Dwarves.age,
    Dwarves.profession, Squads.squad_id, Squads.name AS squad_name,
    Squads.mission
FROM Dwarves
...
Задание 18.4
SELECT Dwarves.dwarf_id, Dwarves.name, COUNT(Items.item_id) AS "Items count"
FROM Dwarves
INNER JOIN Items ON Dwarves.dwarf_id = Items.owner_id
GROUP BY Dwarves.dwarf_id, Dwarves.name;

Задание 18.5
SELECT Squads.squad_id, Squads.name AS squad_name, Squads.mission,
...
Задание 18.6
SELECT Dwarves.profession, COUNT(Tasks.task_id) AS "Unfinished tasks"
FROM Dwarves
...
Задание 18.7
SELECT Items.type, AVG(Dwarves.age) AS "Average owner age"
FROM Items
